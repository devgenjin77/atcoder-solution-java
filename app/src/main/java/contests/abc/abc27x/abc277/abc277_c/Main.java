 ...
 *
 * verified:
 * - https://atcoder.jp/contests/abc277/submissions/36465093
 *
 */
...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    sc.close();
    final List<Integer> list_l = new ArrayList<>(hset);
    Collections.sort(list_l);
    DisjointSetUnion dsu = new DisjointSetUnion(list_l.size());
    for (int i = 0; i < n; i++) {
      int idx_a = Collections.binarySearch(list_l, array_a[i]);
      int idx_b = Collections.binarySearch(list_l, array_b[i]);
      dsu.merge(idx_a, idx_b);
    }
    int ans = 1;
    for (int i = list_l.size() - 1; i > 0; i--) {
      if (dsu.isSame(0, i)) {
        ans = Math.max(list_l.get(i), ans);
        break;
      }
    }
