 ...
 *
 * verified:
 * - https://atcoder.jp/contests/abc285/submissions/38099861
 *
 * note:
 *
 *
...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    final String[] array_t = new String[n];
    final Map<String, Integer> id_map = new HashMap<>();
    for (int i = 0; i < n; i++) {
      array_s[i] = sc.next();
      array_t[i] = sc.next();
      if (!id_map.containsKey(array_s[i])) {
        id_map.put(array_s[i], id_map.size());
      }
      if (!id_map.containsKey(array_t[i])) {
        id_map.put(array_t[i], id_map.size());
      }
    }
    sc.close();
    final DisjointSetUnion dsu = new DisjointSetUnion(id_map.size());
    boolean isOK = true;
    for (int i = 0; i < n; i++) {
      int id1 = id_map.get(array_s[i]);
      int id2 = id_map.get(array_t[i]);
      if (dsu.isSame(id1, id2)) {
        isOK = false;
        break;
      } else {
        dsu.merge(id1, id2);
