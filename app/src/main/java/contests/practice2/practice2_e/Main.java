 ...
 *
 * verified:
 * - https://atcoder.jp/contests/practice2/submissions/31307508
 *
 */

package contests.practice2.practice2_e;
...
public class Main {
  ...
  final static long MAX = 1_000_000_000l;
  ...
  public static void main(String[] args) throws Exception {
    ...
    graph.addEdge(s, t, n * k, MAX);
    for (int i = 0; i < n; i++) {
      graph.addEdge(s, i, k, 0);
      graph.addEdge(n + i, t, k, 0);
    }
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        long a = Long.parseLong(sc.next());
        graph.addEdge(i, n + j, 1, MAX - a);
      }
    }
    sc.close();
    MinCostFlowResult result = graph.flow(s, t, n * k);
    PrintWriter pw = new PrintWriter(System.out);
    pw.println(1L * n * k * MAX - result.cost);
    StringBuilder[] grid_buf = new StringBuilder[n];
    char[] line = new char[n];
    Arrays.fill(line, '.');
    for (int i = 0; i < n; i++) {
      grid_buf[i] = new StringBuilder(new String(line));
    }
    for (MinCostFlowGraphEdge edge : graph.edges()) {
      if (edge.from == s || edge.to == t || edge.flow == 0) {
        continue;
      } else {
        grid_buf[edge.from].setCharAt(edge.to - n, 'X');
      }
    }
    for (StringBuilder str_buf : grid_buf) {
      pw.println(str_buf.toString());
    }
    pw.close();
  }
